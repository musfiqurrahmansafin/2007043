...
public class CityManager {
    ...
    public List<City> getCities(String sortType) {
        ...
        if ("name".equalsIgnoreCase(sortType)) {
            sortedList.sort(Comparator.comparing(City::getName));
        } else if ("province".equalsIgnoreCase(sortType)) {
            sortedList.sort(Comparator.comparing(City::getProvince));
        }
        return sortedList;
    }

    // Method to add a city to the list, for completeness
